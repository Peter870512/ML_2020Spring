...
np.save('train_mean.npy',mean_x)
np.save('train_std.npy',std_x)
'''
train_x = math.floor(len(x) * 0.9)
train_y = math.floor(len(y) * 0.9)

x_train_set = x[: train_x, :].copy()
y_train_set = y[: train_y, :].copy()
x_validation = x[ train_x: , :].copy()
y_validation = y[ train_y: , :].copy()

x_train_set = np.concatenate((x_train_set, x_train_set ** 2), axis = 1).astype(float)
x_train_set = np.concatenate((np.ones([train_x, 1]), x_train_set), axis = 1).astype(float)
'''
dim = 2 * 8 * 9 + 1
x = np.concatenate((x, x ** 2), axis = 1).astype(float)
...
for t in range(iter_time):
    ...
    '''
    loss = np.sqrt(np.sum(np.power(np.dot(x_train_set, w) - y_train_set, 2))/ train_x) #rmse 
    if(t%1000==0):
        print(str(t) + ":" + str(loss))
...
'''
# Validation
x_validation = np.concatenate((x_validation, x_validation ** 2 ), axis = 1).astype(float)
